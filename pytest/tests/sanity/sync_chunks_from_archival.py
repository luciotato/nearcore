...
while True:
    ...
    new_height = status['sync_info']['latest_block_height']
    if new_height > height_to_sync_to:
        break
    time.sleep(1)
...
for h in range(10, start_height):
    assert (h, 0, 2) in requests
    assert (h, 0, 2) in responses
    assert (h, 1, 2) not in requests

    assert (h, 1, 3) in requests
    assert (h, 1, 3) in responses
    assert (h, 0, 3) not in requests
...
for h in range(new_height - 5, new_height - 1):
    assert (h, 0, 2) in requests
    assert (h, 0, 2) in responses
    assert (h, 1, 2) in requests
    assert (h, 1, 2) in responses

    assert (h, 1, 3) in requests
    assert (h, 1, 3) in responses
    assert (h, 0, 3) in requests
    assert (h, 0, 3) in responses
